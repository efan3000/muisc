...
class PreTrainedModel(nn.Module, ModuleUtilsMixin, GenerationMixin, PushToHubMixin):
    ...
    def _load_state_dict_into_model(
        ...
        #print('xc====== _load_state_dict_into_model: expected_keys = {}, loaded_keys = {}'.format(expected_keys, loaded_keys))
...
class Conv1D(nn.Module):
    ...
    def forward(self, x):
        #print('Conv1D: 1. bias = {}, x = {}, weight = {}'.format(self.bias.shape, x.shape,  self.weight.shape))
...
class SequenceSummary(nn.Module):
    ...
    def __init__(self, config: PretrainedConfig):
        ...
        if hasattr(config, "summary_use_proj") and config.summary_use_proj:
            if hasattr(config, "summary_proj_to_labels") and config.summary_proj_to_labels and config.num_labels > 0:
                ...
                print('1. config.num_labels = {}'.format(config.num_labels))
            else:
                num_classes = config.hidden_size
                print('2. config.hidden_size = {}'.format(config.hidden_size))
            print('xc====== SequenceSummary: num_classes = {}, config.hidden_size = {}'.format(num_classes, config.hidden_size))
    ...
    def forward(
        ...
        """
        #print('xc====== SequenceSummary: hidden_states = {}'.format(hidden_states))
        if self.summary_type == "last":
            output = hidden_states[:, -1]
        elif self.summary_type == "first":
            #print('xc====== SequenceSummary: first.')
            output = hidden_states[:, 0]
            #print("hidden_states = {}".format(hidden_states[:, 0, :10]))
        elif self.summary_type == "mean":
            output = hidden_states.mean(dim=1)
        elif self.summary_type == "cls_index":
            #print('xc====== SequenceSummary: cls_index = {}'.format(cls_index))
        ...
        #print('xc====== SequenceSummary: 1. output = {}'.format(output.shape))
        output = self.summary(output)
        #print('xc====== SequenceSummary: 2. output = {}'.format(output.shape))
        output = self.activation(output)
        output = self.last_dropout(output)
        #print('xc====== SequenceSummary: done. output = {}'.format(output))
